...
1. Fork this repo
2. Connect to Netlify via **Import from GitHub**
...
2. **Parent Message Drafter** — describe the detection, Claude writes the perfect SMS or email to send to a parent
3. **Suspect Behavior Profiler** — analyze all flagged incidents from a suspect's profile with full psychological assessment
4. **Grooming Stage Predictor** — identify exactly which of 6 grooming stages a message represents, with visual pipeline and next-step prediction
5. **Multi-Language Detector** — detect grooming in 80+ languages (Spanish, Arabic, Mandarin, Portuguese, etc.) — fills the gap the keyword scanner leaves
